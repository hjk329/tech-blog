...
FID 메트릭은 실제 사용자가 있어야 측정할 수 있고, 사용자마다 상대적인 경험을 제공할 수도 있기 때문에 측정이 애매할 수 있다고 생각했습니다.  
...
화살표 이미지의 src가 빈 문자열이었다가 렌더링이 완료된 후 path 및 스크롤 여부에 따라서 src가 변경되면서 화면이 버벅이는 것처럼 보였던 것입니다.  
...
정리하자면 크기가 정해지지 않은 이미지나 비디오 요소는 CLS를 유발하기 때문에 이 점을 유의해주시면 됩니다.  
